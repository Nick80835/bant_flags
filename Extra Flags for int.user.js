...
var setup = {
  ...
  html: function () {
    ...
    var htmlSaveButton = '<div><button name="save" title="Pressing &#34;Save Regions&#34; will set your regions to the ones current displayed below.">' + "Save Regions</button></div><br/>"
    var htmlHelpText =
      '<label name="' +
      shortId +
      'label"> You can go as deep as you like, regions stack.<br/>' +
    ...
    if (regions.length > 1) {
      ...
      selectMenuFlags += "<br/>"
      return htmlFixedStart + '<div>Region: <br/><select id="' + shortId + 'countrySelect">' + "</select></div><br/>" + htmlBackNextButtons + "<br/>" + htmlSaveButton + "</div>" + selectMenuFlags + htmlHelpText + filterRadio
    }

    if (regions.length == 1) {
      var selectMenuFlags = "<br/>"
      return htmlFixedStart + '<div>Region: <br/><select id="' + shortId + 'countrySelect">' + "</select></div><br/>" + htmlBackNextButtons + "<br/>" + "</div><br/><br/>" + selectMenuFlags + htmlHelpText + filterRadio
    }

    return htmlFixedStart + '<div>Country: <br/><select id="' + shortId + 'countrySelect">' + "</select></div><br/>" + htmlBackNextButtons + "<br/>" + htmlHelpText + filterRadio
  },
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (response) {
        if (response.status == 404) {
          // detect if there are no more folders
          ...
          for (var countriesCounter = 0; countriesCounter < countriesAvailable.length - 1; countriesCounter++) {
            ...
            if (lastRegion != "" && countriesAvailable[countriesCounter] === lastRegion) {
              // automatically select last selected when going up a folder
              opt.selected = "selected"
            } else if (oldPath == "" && countriesAvailable[countriesCounter] === regions[regions.length - 1]) {
              // show final selected when no more
              // folders detected
              opt.selected = "selected"
            }
  ...
  show: function () {
    ...
    GM_addStyle(
      "\
            #" +
        setup.id +
        " { position:fixed;z-index:10001;top:40px;right:40px;padding:20px 30px;background-color:white;width:auto;border:1px solid black }\
            #" +
        setup.id +
        " * { color:black;text-align:left;line-height:normal;font-size:12px }\
            #" +
        setup.id +
        " div { text-align:center;font-weight:bold;font-size:14px }"
    )
    ...
    setup.q("forward").addEventListener(
      "click",
      function () {
    ...
    setup.q("save").addEventListener(
      ...
      function () {
        ...
        if (regions[regions.length - 1] === "") {
          //prevent last spot from being blank
          regions.pop()
        }
        lastRegion = ""
  ...
  init: function () {
    ...
    function setEnabled() {
      switch (GM_getValue("enabled", true)) {
        case true: {
    ...
    if (postedRegions.length > 0 && !(currentFlag === undefined)) {
      ...
      for (var i = 0; i < postedRegions.length; i++) {
        ...
        if (radio === "all" || (radio === "first" && i === 0) || (radio === "last" && i === postedRegions.length - 1)) {
          ...
          var newFlagImgOpts =
            "onerror=\"(function () {var extraFlagsImgEl = document.getElementById('pc" +
            post.post_nr +
            "').getElementsByClassName('extraFlag')[" +
            lastI +
          ...
          newFlag.style = "padding: 0px 0px 0px 5px; width: 16px; height: 11px; position: relative;"
    ...
    if (GM_getValue("enabled", true)) {
      setTimeout(function () {
        GM_xmlhttpRequest({
          ...
          data: "post_nr=" + encodeURIComponent(e.detail.postID) + "&" + "board=" + encodeURIComponent(e.detail.boardID) + "&" + "regions=" + encodeURIComponent(regions.slice(1).join(regionDivider)),
          headers: {
            "Content-Type": "application/x-www-form-urlencoded"
          },
          onload: function (response) {
    ...
    if (GM_getValue("enabled", true)) {
      ...
      setTimeout(function () {
        GM_xmlhttpRequest({
          method: "POST",
          url: backendBaseUrl + postUrl,
          data: "post_nr=" + encodeURIComponent(evDetail.postId) + "&" + "board=" + encodeURIComponent(boardID) + "&" + "regions=" + encodeURIComponent(regions.slice(1).join(regionDivider)),
          headers: {
            "Content-Type": "application/x-www-form-urlencoded"
          },
          onload: function (response) {
